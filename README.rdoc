...
RightSupport::Config contains module FeaturesAdmitter, which provide possibility 
...
Then, if you would mix RightSupport::Config::FeaturesAdmitter in you class, like:

  class SweetestClass
    include RightSupport::Config::FeaturesAdmitter
    self.yaml_config = '/tmp/features_config.yml'      
  end

* SweetestClass.yaml_config could be file path, io or string.

You will have access now to methods, like:

* SweetestClass.feature_speak_belarusian_allowed?
* SweetestClass#feature_speak_klingonese_allowed?

Consider:
1) it will rewrite your own class method_missing, so, please, be careful with it;
2) it is evaluate everything that is not true as false;
