...
RightSupport::Config contains class functionality, which provides possibility 
...
Then, if you would add configuration in you class, like:
  ...
  class SweetestClass    
    CONFIG = RightSupport::Config.read('/tmp/features_config.yml')
  end

* RightSupport::Config.read receives file` path, io or string.

Now you can call CONFIG['feature_group']['feature'], like:

* SweetestClass::CONFIG['eat']['khlav kalash']
* SweetestClass::CONFIG['speak']['belarusian']

Consider it evaluates everything that is not true as false.
