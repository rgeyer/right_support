...
module RightSupport::Net
  ...
  #
  # To start use it, all you need is to register DNS name of the host, which you will trust for sure.
  # This module automatically adds OpenSSL library and reopens verify_certificate_identity method, to
  # change the SSL algorythm of hostname verification.
  module DNSResolver
    class << self
      @@registered_hostnames_hash = {}

      def get_trusted_dns_name(ip)
        @@registered_hostnames_hash[ip]
      end

      def register_trusted_dns_name(hostname)
        trust_dns_completely
        begin
      ...
      def trust_dns_completely
        require 'openssl'
        OpenSSL::SSL.module_exec do
          def verify_certificate_identity(cert, hostname)
            actual_hostname = RightSupport::Net::DNSResolver.get_trusted_dns_name(hostname)
